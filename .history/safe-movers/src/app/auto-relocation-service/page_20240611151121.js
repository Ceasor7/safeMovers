...
const autoRelocationDescription = [
  {
    title: 'Local Vehicle Relocation across Kenya',
    description:
      'Our local vehicle relocation services within Kenya are designed to simplify the process for clients looking to move their cars across cities or regions. With a meticulous approach, we handle every aspect of the relocation journey, from the initial pick-up to the final delivery. Our experienced team ensures the safe transport of vehicles, employing state-of-the-art handling techniques and secure transport methods. Whether it’s a single car or a fleet, our focus remains on timely and secure deliveries while adhering to the highest safety standards',
  },
  {
    title: 'International Auto Shipping',
    description:
      'For clients seeking to transport vehicles outside the African continent, Safe Movers provides international auto shipping services. Our global reach and partnerships with renowned logistics companies guarantee a smooth transition from one country to another, no matter the distance. From documentation and customs clearance to selecting the most suitable shipping methods, our team handles every detail to ensure a stress-free international vehicle relocation experience.',
  },
  {
    title: 'Cross-Border Transport',
    description:
      'Expanding beyond Kenya’s borders, Safe Movers facilitates cross-border transport across neighboring countries and Africa. Our comprehensive logistics network and expertise enable us to seamlessly transport vehicles to various countries within East Africa and beyond. Compliance with international regulations and customs requirements is a cornerstone of our service, ensuring a hassle-free experience for our clients. We pride ourselves on navigating the complexities of cross-border logistics, offering reliable and efficient solutions for vehicle relocation.',
  },
];
    ...
    <div className="my-5  mx-auto max-w-5xl items-center">
      ...
      <div className="flex flex-col sm:flex-row sm:gap-x-10">
        <div className="sm:w-2/3 flex flex-col">
          ...
          {autoRelocationDescription.map((item, index) => (
            <div key={index}>
              <div>
          ...
          <div className="my-7">
            ...
            <div className="my-7">
              {whyChooseSafeMovers.map((item, index) => (
                <div key={index}>
                  <div>
                    <h2 className="font-bold my-2 text-lg">{item.title}</h2>
                  </div>
